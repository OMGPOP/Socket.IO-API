...
socketApi.mapCall("user.initialize", function(data) {
  $('body').append($('<div>').html("logged in as: " + data['user_id']))
  socketApi.send("user.join", { room: "cwh" })
})

socketApi.mapCall("user.joined", function(data) { })

socketApi.mapCall("user.create", function(data) {
	$('body').append($('<div>').html("created user: " + data['user_id']))
	$('body').append($('<div>', {id:'box_' + data['user_id'],'class':'avatar_a'}))
})

socketApi.mapCall("user.destroy", function(data) {
